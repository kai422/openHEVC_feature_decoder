...
{
    // get decoded frame output
    OpenHevcWrapperContexts *openHevcContexts = (OpenHevcWrapperContexts *) openHevcHandle;
    OpenHevcWrapperContext  *openHevcContext  = openHevcContexts->wraper[openHevcContexts->display_layer];

    // YUV data are saved in openHevcContext->picture->data
    if (got_picture) {
...
int libOpenHevcGetOutputCpy(OpenHevc_Handle openHevcHandle, int got_picture, OpenHevc_Frame_cpy *openHevcFrame)
{
    ...
    if( got_picture ) {
        ...
        //MvDecoder
        unsigned char *MV = (unsigned char *) openHevcFrame->pvMV;
        unsigned char *YR = (unsigned char *) openHevcFrame->pvYR;
        unsigned char *UR = (unsigned char *) openHevcFrame->pvUR;
        unsigned char *VR = (unsigned char *) openHevcFrame->pvVR;
        ...
        coded_height = *(&openHevcContext->c->coded_height);
        ...
        int src_stride_pu_x2 = (src_stride >> 2) * 2; //int16_t
        int dst_stride_pu_x2 = (dst_stride >> 2) * 2; //int16_t
        y_offset = y_offset2 = 0;

        //l0_mx
        for (y = 0; y < coded_height >> 2 ; y++){
            if (y < height >>2){
                memcpy(&MV[y_offset2], &openHevcContext->picture->data[3][y_offset], dst_stride_pu_x2);
                y_offset2 += dst_stride_pu_x2;
            }
            y_offset  += src_stride_pu_x2;
        }
        ...
        int src_stride_pu = src_stride >> 2;
        int dst_stride_pu = dst_stride >> 2;

        //l0_ref
        for (y = 0; y < coded_height >> 2 ; y++){
            if (y<height >> 2){
                memcpy(&MV[y_offset2], &openHevcContext->picture->data[3][y_offset], dst_stride_pu);
                y_offset2 += dst_stride_pu;
            }
            y_offset  += src_stride_pu;
        }
        ...
        int src_stride_cu = src_stride >> 3;
        int dst_stride_cu = dst_stride >> 3;

        //size
        for (y = 0; y < height >> 3; y++){
            memcpy(&MV[y_offset2], &openHevcContext->picture->data[3][y_offset], dst_stride_cu);
            y_offset  += src_stride_cu;
            y_offset2 += dst_stride_cu;
        }
        //quadtree
        memcpy(&MV[3 * dst_stride * height>>2], &openHevcContext->picture->data[3][3 * src_stride * coded_height>>2], dst_stride*height>>2);
        memset(&openHevcContext->picture->data[3][0],0, src_stride * coded_height); // clean the buffer



        y_offset = y_offset2 = 0;
        int src_stride_int16_t = src_stride << 1;
        int dst_stride_int16_t = dst_stride << 1;

        for (y = 0; y < height; y++) {
            memcpy(&YR[y_offset2], &(openHevcContext->picture->data[4])[y_offset], dst_stride_int16_t);
            y_offset  += src_stride_int16_t;
            y_offset2 += dst_stride_int16_t;
        }

        y_offset = y_offset2 = 0;

        int src_stride_int16_t_c = src_stride << 1;
        int dst_stride_int16_t_c = dst_stride << 1;

        for (y = 0; y < height >> format; y++) {
            memcpy(&UR[y_offset2], &(openHevcContext->picture->data[5])[y_offset], dst_stride_int16_t_c);
            memcpy(&VR[y_offset2], &(openHevcContext->picture->data[6])[y_offset], dst_stride_int16_t_c);
            y_offset  += src_stride_int16_t_c;
            y_offset2 += dst_stride_int16_t_c;
        }
   }
    return 1;
}
