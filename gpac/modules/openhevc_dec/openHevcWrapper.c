...
int libOpenHevcGetOutputCpy(OpenHevc_Handle openHevcHandle, int got_picture, OpenHevc_Frame_cpy *openHevcFrame)
{
    ...
    if( got_picture ) {
        ...
        for (y = 0; y < height; y++) {
            memcpy(&YR[y_offset2], &openHevcContext->picture->data[4][y_offset], dst_stride);
            y_offset  += src_stride;
            y_offset2 += dst_stride;
        }
        ...
        for (y = 0; y < height >> format; y++) {
            memcpy(&UR[y_offset2], &openHevcContext->picture->data[5][y_offset], dst_stride_c);
            memcpy(&VR[y_offset2], &openHevcContext->picture->data[6][y_offset], dst_stride_c);
            y_offset  += src_stride_c;
            y_offset2 += dst_stride_c;
        }

        memset(&openHevcContext->picture->data[4][0],0, src_stride * height); // clean the buffer
        memset(&openHevcContext->picture->data[5][0],0, src_stride_c * height >> format); // clean the buffer
        memset(&openHevcContext->picture->data[6][0],0, src_stride_c * height >> format); // clean the buffer
    }
    return 1;
}
