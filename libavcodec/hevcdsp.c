...
#define HEVC_DSP(depth)                                                            \
	...
	hevcdsp->transform_add[2]       = FUNC(transform_add16x16, depth);             \
	hevcdsp->transform_add[3]       = FUNC(transform_add32x32, depth);             \
    hevcdsp->transform_get_dct[0]   = FUNC(transform_get_dct4x4, depth);           \
	hevcdsp->transform_get_dct[1]   = FUNC(transform_get_dct8x8, depth);           \
	hevcdsp->transform_get_dct[2]   = FUNC(transform_get_dct16x16, depth);         \
	hevcdsp->transform_get_dct[3]   = FUNC(transform_get_dct32x32, depth);         \
	hevcdsp->transform_skip         = FUNC(transform_skip, depth);                 \
	hevcdsp->transform_rdpcm        = FUNC(transform_rdpcm, depth);                \
	hevcdsp->idct_4x4_luma          = FUNC(transform_4x4_luma, depth);             \
																				   \
